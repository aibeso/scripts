...
!(async () => {
    ...
    if (cookiesArr.length > 1){
        ...
        for (let i = 0; i < cookiesArr.length; i++) {
            ...
            $.UserName = decodeURIComponent(cookie.match(/pt_pin=([^; ]+)(?=;?)/) && cookie.match(/pt_pin=([^; ]+)(?=;?)/)[1])
            $.canRun = true
            console.log(`\n${$.UserName} 开始助力脚本内置助力码\n`)
            for (let j = 0; j < inCode.length; j++) {
                let item = {
                    redEnvelopeId:'996c98ca1d2041e0a1fb7b4ff65560a839921631052130027',
                    inviter:'DN4SmaAmNG_MAYReZQ4TPw',
                }
                await help(item.redEnvelopeId, item.inviter, 1)
                if (!$.canRun) {
                    break;
                ...
                if (err) {
                    ...
                    if (data.data && data.data.helpResult) {
                        ...
                        if (data.data.helpResult.code === 16005 || data.data.helpResult.code === 16007) {
                            ...
                            $.needhelp = false
                        } else if (data.data.helpResult.code === 16013 || data.data.helpResult.code === 16012){
                            $.canHelp = false
                        }
                    } else {
